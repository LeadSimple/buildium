...
module Buildium
  class ApplicationSectionResponseMessage
    attr_accessor :section_fields
    ...
    def self.openapi_types
      {
        :'section_fields' => :'Array<ApplicationResponseFieldMessage>'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Buildium::ApplicationSectionResponseMessage` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Buildium::ApplicationSectionResponseMessage`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'section_fields')
        if (value = attributes[:'section_fields']).is_a?(Array)
          self.section_fields = value
        end
      end
    end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          section_fields == o.section_fields
    end
