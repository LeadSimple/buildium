...
module Buildium
  class AssociationArchitecturalRequestMessage
    ...
    def self.openapi_types
      {
        ...
        :'created_by_user' => :'AssociationArchitecturalRequestMessageCreatedByUser',
        :'last_updated_date_time' => :'Time',
        :'last_updated_by_user' => :'AssociationArchitecturalRequestMessageLastUpdatedByUser'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Buildium::AssociationArchitecturalRequestMessage` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Buildium::AssociationArchitecturalRequestMessage`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      status_validator = EnumAttributeValidator.new('String', ["New", "InProgress", "Completed"])
      return false unless status_validator.valid?(@status)
      decision_validator = EnumAttributeValidator.new('String', ["Pending", "Approved", "Denied"])
    ...
    def status=(status)
      validator = EnumAttributeValidator.new('String', ["New", "InProgress", "Completed"])
    ...
    def decision=(decision)
      validator = EnumAttributeValidator.new('String', ["Pending", "Approved", "Denied"])
      unless validator.valid?(decision)
        fail ArgumentError, "invalid value #{ decision.inspect } for \"decision\", must be one of #{validator.allowable_values}."
      end
      @decision = decision
    end
    ...
    def hash
      [id, association_id, ownership_account_id, name, submitted_date_time, status, decision, created_date_time, created_by_user, last_updated_date_time, last_updated_by_user].hash
    end
