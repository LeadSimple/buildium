...
module Buildium
  class AssociationOwnershipAccountSearchMessage
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Buildium::AssociationOwnershipAccountSearchMessage` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Buildium::AssociationOwnershipAccountSearchMessage`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'association_ids')
        if (value = attributes[:'association_ids']).is_a?(Array)
          self.association_ids = value
        end
      end

      if attributes.key?(:'unit_or_owner')
        self.unit_or_owner = attributes[:'unit_or_owner']
      end
    ...
    def hash
      [ids, association_ids, unit_or_owner, date_from, date_to, status, delinquency_statuses].hash
    end
