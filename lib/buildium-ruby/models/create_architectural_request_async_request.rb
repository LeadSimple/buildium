...
module Buildium
  class CreateArchitecturalRequestAsyncRequest
    ...
    # The date and time the architectural request was submitted. Must not be in the future.
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Buildium::CreateArchitecturalRequestAsyncRequest` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Buildium::CreateArchitecturalRequestAsyncRequest`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [association_id, ownership_account_id, name, submitted_date_time, status, decision].hash
    end
