...
module Buildium
  class CreateContactRequestTaskRequest
    ...
    def self.openapi_types
      {
        ...
        :'priority' => :'String',
        :'due_date' => :'Date',
        :'contact_detail' => :'ContactRequestTaskPostMessageContactDetail'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Buildium::CreateContactRequestTaskRequest` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Buildium::CreateContactRequestTaskRequest`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [title, description, category_id, sub_category_id, property_id, unit_id, assigned_to_user_id, task_status, priority, due_date, contact_detail].hash
    end
