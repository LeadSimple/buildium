...
module Buildium
  # An object that contains search criteria for retrieving general ledger account balances.
  class GLAccountBalanceSearchMessage
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Buildium::GLAccountBalanceSearchMessage` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Buildium::GLAccountBalanceSearchMessage`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      entity_type_validator = EnumAttributeValidator.new('String', ["Association", "Rental", "RentalOwner"])
    ...
    def hash
      [entity_type, entity_id, gl_account_ids, accounting_basis, as_of_date].hash
    end
