...
module Buildium
  class LeaseOutstandingBalanceSearchMessage
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Buildium::LeaseOutstandingBalanceSearchMessage` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Buildium::LeaseOutstandingBalanceSearchMessage`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'lease_ids')
        if (value = attributes[:'lease_ids']).is_a?(Array)
          self.lease_ids = value
        end
      end
      ...
      if attributes.key?(:'eviction_status')
        self.eviction_status = attributes[:'eviction_status']
      end
    end
    ...
    def valid?
      ...
      eviction_status_validator = EnumAttributeValidator.new('String', ["NotEvictionPending", "EvictionPending"])
      return false unless eviction_status_validator.valid?(@eviction_status)
      true
    end
    ...
    # Custom attribute writer method checking allowed values (enum).
    # @param [Object] eviction_status Object to be assigned
    ...
    def hash
      [entity_type, entity_id, lease_statuses, lease_ids, past_due_email, balance_duration, eviction_status].hash
    end
