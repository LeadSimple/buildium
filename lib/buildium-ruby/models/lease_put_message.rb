  ...
  class LeasePutMessage
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Buildium::LeasePutMessage` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Buildium::LeasePutMessage`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @is_eviction_pending.nil?
        invalid_properties.push('invalid value for "is_eviction_pending", is_eviction_pending cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @is_eviction_pending.nil?
      true
    end
    ...
    def hash
      [lease_type, unit_id, lease_from_date, lease_to_date, is_eviction_pending, automatically_move_out_tenants].hash
    end
