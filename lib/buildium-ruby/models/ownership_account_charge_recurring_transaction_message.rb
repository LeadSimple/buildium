...
module Buildium
  class OwnershipAccountChargeRecurringTransactionMessage
    # The unique identifier of the recurring charge schedule.
    attr_accessor :id

    # The unique identifier of the ownership account that the recurring charge will be applied to.
    ...
    # The unique identifier of the scheduled Rent entity. If the charge is not associated with a Rent entity then the value will be `NULL`.
    ...
    # Specifies the period of time/occurrences the recurring charge will be processed. Note, if the `Frequency` field is set to `OneTime` this field should be set to `NULL` as any submitted value will be ignored.
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Buildium::OwnershipAccountChargeRecurringTransactionMessage` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Buildium::OwnershipAccountChargeRecurringTransactionMessage`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [id, ownership_account_id, gl_account_id, rent_id, amount, memo, first_occurrence_date, next_occurrence_date, post_days_in_advance, occurrences_remaining, frequency, duration].hash
    end
