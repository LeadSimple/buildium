...
module Buildium
  ...
  class RentalOwnerMessage
    ...
    # Start date of the management agreement with the rental owner. Null if value is not set.
    attr_accessor :management_agreement_start_date

    # End date of the management agreement with the rental owner. Null if value is not set.
    attr_accessor :management_agreement_end_date
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Buildium::RentalOwnerMessage` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Buildium::RentalOwnerMessage`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [id, is_company, is_active, first_name, last_name, phone_numbers, email, alternate_email, comment, address, management_agreement_start_date, management_agreement_end_date, company_name, property_ids, tax_information].hash
    end
