...
module Buildium
  class ResidentRequestTaskMessage
    ...
    # Indicates whether the resident has permitted entry. A null value represents no response was provided from the resident.
    attr_accessor :is_entry_permitted_by_resident

    # Indicates whether the resident has pets. A null value represents no response was provided from the resident.
    attr_accessor :does_resident_have_pets
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Buildium::ResidentRequestTaskMessage` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Buildium::ResidentRequestTaskMessage`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'appliance')
        self.appliance = attributes[:'appliance']
      end
    ...
    def hash
      [id, category, title, description, property, unit_id, unit_agreement, requested_by_user_entity, assigned_to_user_id, task_status, priority, due_date, created_date_time, last_updated_date_time, appliance, is_entry_permitted_by_resident, does_resident_have_pets, resident_entry_notes].hash
    end
