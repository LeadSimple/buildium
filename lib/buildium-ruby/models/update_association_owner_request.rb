...
module Buildium
  class UpdateAssociationOwnerRequest
    ...
    def self.openapi_types
      {
        ...
        :'phone_numbers' => :'AssociationOwnerPutMessagePhoneNumbers',
        :'emergency_contact' => :'AssociationOwnerPostMessageEmergencyContact',
        :'comment' => :'String',
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Buildium::UpdateAssociationOwnerRequest` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Buildium::UpdateAssociationOwnerRequest`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [first_name, last_name, primary_address, alternate_address, email, alternate_email, phone_numbers, emergency_contact, comment, mailing_preference, tax_id].hash
    end
