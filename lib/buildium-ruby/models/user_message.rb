...
module Buildium
  ...
  class UserMessage
    ...
    # Date and time the user last logged into Buildium. This value will be `NULL` if the user has never logged into Buildium.
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Buildium::UserMessage` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Buildium::UserMessage`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [id, user_types, is_active, last_login, first_name, last_name, company_name, email, alternate_email, phone_numbers, user_role, is_company].hash
    end
